// =============================================================================
//  ApiMonitor — records network responses (page.on('response') / 'requestfailed')
//  and surfaces failed (>=400), server-error (>=500), and slow API calls.
//  Scopes to same-origin XHR/fetch so third-party widgets don't pollute results.
// =============================================================================
...
export class ApiMonitor {
  ...
  private readonly serverErrors: FailedRequest[] = [];
  ...
  constructor(
    ...
    page.on('response', (res: Response) => {
      ...
      if (status >= 400) {
        const entry = { url, status, method: req.method() };
        this.failed.push(entry);
        if (status >= 500) this.serverErrors.push(entry); // always-fail subset
      }
  ...
  /** 5xx responses only (the always-fail subset). */
  getServerErrors(): FailedRequest[] {
