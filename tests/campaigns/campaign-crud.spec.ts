 ...
 * playlist to open. It borrows an existing one read-only and never saves it —
 * cms2 is shared, and at least one playlist there is explicitly marked
 * "do not change or delete".
 */
async function anyPlaylistId(page: import('@playwright/test').Page): Promise<string> {
  await page.goto('/playlists', { waitUntil: 'domcontentloaded' });
  const link = page.locator('a[href^="/playlist-settings/"]').first();
  await expect(link, 'cms2 must have at least one playlist to host the picker').toBeVisible({
    timeout: 30_000,
  });
...
test.describe('Campaigns · CRUD', () => {
  ...
  test('CMP-001 · create a campaign through the picker @smoke @critical @ui', async ({
    ...
    await campaignPicker.open(await anyPlaylistId(page));
