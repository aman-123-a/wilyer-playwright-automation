 ...
 */
async function openPicker(prayerSchedulePage: PrayerSchedulePage): Promise<boolean> {
  await prayerSchedulePage.open();
  await prayerSchedulePage.openSchedules();

  // The per-prayer table renders after the plan detail loads; PrayerSchedulePage
  // owns that wait (a non-retrying isVisible() probe would false-skip here).
  if (!(await prayerSchedulePage.prayerTableReady())) return false;
...
async function intoNestedFolder(psp: PrayerSchedulePage): Promise<void> {
...
test.describe('Prayer Schedule — Choose Media picker', () => {
  ...
  test('FILES column opens the Choose Media picker @smoke @sanity @regression', async ({
    ...
    test.skip(!(await openPicker(prayerSchedulePage)), 'no prayer plan with a FILES column');
  ...
  test('a three-level nested path is reachable @regression', async ({ prayerSchedulePage }) => {
    test.skip(!(await openPicker(prayerSchedulePage)), 'no prayer plan with a FILES column');
  ...
  test('search stays scoped to the folder subtree @regression', async ({ prayerSchedulePage }) => {
    test.skip(!(await openPicker(prayerSchedulePage)), 'no prayer plan with a FILES column');
    await intoNestedFolder(prayerSchedulePage);
  ...
  test('a no-match search shows an empty state @regression', async ({ prayerSchedulePage }) => {
    test.skip(!(await openPicker(prayerSchedulePage)), 'no prayer plan with a FILES column');
    await intoNestedFolder(prayerSchedulePage);
  ...
  test('file/read carries the folderId of the open folder @api @regression', async ({
    ...
    const calls = prayerSchedulePage.recordRequests(/\/v3\/cms\/file\/read/);
  ...
  test('folder/read is issued with a parentFolder on drill-down @api @regression', async ({
    ...
    const calls = prayerSchedulePage.recordRequests(/\/v3\/cms\/folder\/read/);
