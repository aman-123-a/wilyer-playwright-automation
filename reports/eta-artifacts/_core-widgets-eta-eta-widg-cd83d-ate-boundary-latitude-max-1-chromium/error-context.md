...
- Name: _core\widgets\eta\eta-widget.spec.ts >> Live ETA widget — BVA @boundary >> ETA-035-lat-over coordinate boundary: latitude max+1
- Location: tests\_core\widgets\eta\eta-widget.spec.ts:447:5

# Error details

```
Error: latitude max+1 must be rejected
