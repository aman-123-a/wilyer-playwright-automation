...
- Name: _core\widgets\eta\eta-widget.spec.ts >> Live ETA widget — BVA @boundary >> ETA-038 500 destinations are bounded, not silently stored
- Location: tests\_core\widgets\eta\eta-widget.spec.ts:484:3

# Error details

```
Error: unbounded destinations means unbounded routing spend on every refresh
